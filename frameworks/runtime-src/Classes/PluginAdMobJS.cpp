...
#endif
#if defined(MOZJS_MAJOR_VERSION)
bool js_PluginAdMobJS_PluginAdMob_setGDPR(JSContext *cx, uint32_t argc, JS::Value *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    bool ok = true;
    if (argc == 1) {
        ...
        JSB_PRECONDITION2(ok, cx, false, "js_PluginAdMobJS_PluginAdMob_setGDPR : Error processing arguments");
        sdkbox::PluginAdMob::setGDPR(arg0);
        args.rval().setUndefined();
        return true;
    }
    JS_ReportErrorUTF8(cx, "js_PluginAdMobJS_PluginAdMob_setGDPR : wrong number of arguments");
    return false;
}
#elif defined(JS_VERSION)
JSBool js_PluginAdMobJS_PluginAdMob_setGDPR(JSContext *cx, uint32_t argc, jsval *vp)
{
    jsval *argv = JS_ARGV(cx, vp);
    JSBool ok = JS_TRUE;
    if (argc == 1) {
...
void js_register_PluginAdMobJS_PluginAdMob(JSContext *cx, JS::HandleObject global) {
...
#else
    static JSClass PluginAdMob_class = {
        "PluginAdMob",
        JSCLASS_HAS_PRIVATE,
        nullptr
    };
    jsb_sdkbox_PluginAdMob_class = &PluginAdMob_class;
#endif
    ...
    static JSFunctionSpec st_funcs[] = {
        ...
        JS_FN("setGDPR", js_PluginAdMobJS_PluginAdMob_setGDPR, 1, JSPROP_PERMANENT | JSPROP_ENUMERATE),
